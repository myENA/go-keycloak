...
type PublicKeyCache interface {
	...
	Load(issuerHost, realm string) (*rsa.PublicKey, bool)
	...
	Store(issueHost, realm string, pk *rsa.PublicKey, ttl time.Duration)
...
func (pkc *debugPublicKeyCache) Load(issuerHost, realm string) (*rsa.PublicKey, bool) {
	pkc.mu.RLock()
...
func (pkc *debugPublicKeyCache) Store(issuerHost, realm string, pk *rsa.PublicKey, _ time.Duration) {
	pkc.mu.Lock()
...
func (pkc *debugPublicKeyCache) Remove(issuerHost, realm string) bool {
	var ok bool
	pkc.mu.Lock()
	key := buildPKCacheKey(issuerHost, realm)
	if _, ok = pkc.m[key]; ok {
		delete(pkc.m, key)
	}
	pkc.mu.Unlock()
	return ok
...
func (pkc *debugPublicKeyCache) Flush() {
	pkc.mu.Lock()
	pkc.m = make(map[string]*rsa.PublicKey)
	pkc.mu.Unlock()
}
...
func NewTimedPublicKeyCache(ctx context.Context, log zerolog.Logger, timedCacheMutators ...sclg.TimedCacheConfigMutator) *TimedPublicKeyCache {
...
func (pkc *TimedPublicKeyCache) Load(issuerHost, realm string) (*rsa.PublicKey, bool) {
...
// existing entry
func (pkc *TimedPublicKeyCache) Store(issuerHost, realm string, pk *rsa.PublicKey, ttl time.Duration) {
